...
public class PredictCurrencyService {
    ...
    public void fit() {
        ...
        List<Double> currencies = currencyService.getDollarCurrencyForLastDays(NUMBER_PREVIOUS_DAYS_FOR_FIT);
        fit(weathers, currencies);
        wasFit = true;
    }

    public void fit(List<Weather> weathers, List<Double> currencies) {
        IntStream.range(0, Math.min(currencies.size(), weathers.size()))
                .forEach(i -> {
                    regression.addData(weathers.get(i).getAvgTemperature(), currencies.get(i));
                });
        wasFit = true;
    }

    public double predict(Weather weather) throws Exception {
        if (!wasFit) {
            fit();
        }
